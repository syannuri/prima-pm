...
await pg.getByText('Cost',{exact:true}).first().click().catch(()=>{});   // level-1 group
await pg.waitForTimeout(1000);
await pg.getByRole('button',{name:'Cash-flow',exact:true}).first().click(); // level-2 pill
await pg.waitForTimeout(2000);
const heading = await pg.getByText('Arus kas',{exact:false}).count();
const hasChart = await pg.locator('svg[aria-label="Cash-flow chart"]').count();
await pg.screenshot({path:`${OUT}/cashflow-nav-open.png`});
console.log(JSON.stringify({ panelHeading: heading>0, hasChart: hasChart>0, errs: errs.length }));
await b.close();
