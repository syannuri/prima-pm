...
const PID='3cbf7d30-cd88-4824-8896-ef058ee13481';
...
await pg.getByRole('button',{name:'Risk',exact:true}).first().click();await pg.waitForTimeout(1400);
// scroll to the Risk Register heading (the card list follows it)
await pg.getByText('Risk Register',{exact:false}).first().scrollIntoViewIfNeeded().catch(()=>{});
await pg.waitForTimeout(500);
await pg.screenshot({path:`${OUT}/reg-risk-cards.png`});
// procurement
await pg.getByRole('button',{name:'Procurement',exact:true}).first().click();await pg.waitForTimeout(1400);
await pg.getByText('Procurement Register',{exact:false}).first().scrollIntoViewIfNeeded().catch(()=>{});
await pg.evaluate(()=>window.scrollBy(0,320)); await pg.waitForTimeout(500);
await pg.screenshot({path:`${OUT}/reg-proc-cards.png`});
await b.close();
