...
const t = (over: Partial<PlatformTenant>): PlatformTenant => ({
  ...
  customDomain: null, isPersonal: false, trialEndsAt: null, createdAt: '2026-01-01', updatedAt: '2026-01-01', memberCount: 0,
...
describe('tenantStats', () => {
  it('returns an empty shape for no tenants', () => {
    expect(tenantStats([])).toEqual({
      ...
      members: 0, personal: 0, planSplit: { TRIAL: 0, PRO: 0, ENTERPRISE: 0 },
  ...
  it('estimates MRR/ARPA from ACTIVE paid tenants only', () => {
    const s = tenantStats([
      ...
      t({ status: 'ACTIVE', plan: 'TRIAL' }),        // 0, not paying
  ...
  it('counts statuses, plans and members over corporate tenants only', () => {
    const s = tenantStats([
      ...
      t({ status: 'PENDING', plan: 'TRIAL', memberCount: 1 }),
      t({ status: 'SUSPENDED', plan: 'PRO', memberCount: 3 }),
      t({ status: 'REJECTED', plan: 'TRIAL', memberCount: 0 }),
      t({ isPersonal: true, plan: 'TRIAL', memberCount: 1 }), // guest sandbox — excluded from corp
    ]);
    ...
    expect(s.planSplit).toEqual({ TRIAL: 2, PRO: 2, ENTERPRISE: 1 });
  });
});
