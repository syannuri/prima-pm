...
export default function MockSCurve({ className }: { className?: string }) {
  ...
  return (
    <svg viewBox="0 0 500 320" className={className} fontFamily="ui-sans-serif, system-ui, sans-serif" role="img" aria-label="Earned-value S-curve with SPI and CPI">
      ...
      <polyline points={pts(1, 1)} fill="none" stroke="#94a3b8" strokeWidth="2.5" strokeDasharray="5 4" strokeLinecap="round" />
      {/* AC (actual cost, rose) to today */}
      <polyline points={pts(0.9, 0.62)} fill="none" stroke="#f43f5e" strokeWidth="2.5" strokeLinecap="round" />
      {/* EV (earned value, blue) to today */}
      <polyline points={pts(0.82, 0.62)} fill="none" stroke="#2563eb" strokeWidth="3" strokeLinecap="round" />
      {/* legend */}
