...
export default function MobileTabBar() {
  ...
  const tabs = [
    { to: '/', label: 'Home', icon: HOME, active: loc.pathname === '/' && !cards },
    { to: '/?view=cards', label: 'Projects', icon: FOLDER, active: loc.pathname === '/' && cards },
    third,
  ];
  return (
    <nav
    ...
    >
      <div className="grid grid-cols-3">
        {tabs.map((t) => (
          <Link key={t.label} to={t.to} className={`flex flex-col items-center gap-1 py-2.5 text-[10px] font-medium transition ${t.active ? 'text-brand-600 dark:text-brand-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}>
