...
- **WBS & Schedule** (predictive) — work-breakdown tree, interactive Gantt (drag to reschedule / link dependencies), schedule **baseline & variance** (tracking Gantt), per-task progress, **weighted progress steps**, **multiple owners per task** (a lead PIC plus co-owners, assigned from the resource pool), and the WBS dictionary.
...
- **Project lifecycle & closure governance** — governed status transitions (**Charter → Active ⇄ On-hold → Closed**, ADMIN/PMO only; on-hold requires a reason). A **methodology-aware closure readiness gate** hard-blocks closing until delivery is complete — *predictive* on WBS schedule 100%, *agile/hybrid* on every backlog item being **Done or Deferred** (item-based, not story points) **plus** a formal acceptance sign-off — else an ADMIN/PMO **force-close** with a mandatory reason, with advisory warnings for open CRs/risks/issues/cost/lessons. A **CLOSED project is read-only** (its frozen BAC and data can't be mutated) with a governed **reopen** (ADMIN/PMO + reason). A **baseline lock** freezes the PMB/BAC — cost lines, WBS tasks and the schedule baseline can't change while locked (progress, actuals and risks still can); unlock is a deliberate, audited ADMIN/PMO action (or an approved change request).
...
- **Reporting Hub** — a dedicated **`/reports`** page that separates *working dashboards* (in-context, interactive) from *formal reports* (point-in-time, exportable), on a **View × Cadence** model (**daily / weekly / monthly / yearly**): **Executive** one-screen portfolio health (KPI band, RAG schedule-health distribution, worst-first project heatmap), a **Portfolio** budget-vs-actual financial roll-up, a **RAID roll-up** across all projects, the formal **Project Report** (schedule / cost / task completion / forecast, with a PM-written narrative, owner list and trend-vs-last-status delta), and **Analytics** (EVM-trend, forecast and agile velocity for any project). A one-click **Steering-Committee Board Pack** PDF assembles portfolio health + top risks + open issues + decisions needed.
...
- **Multi-tenant workspaces** — pooled multitenancy with fail-closed tenant scoping on every query, per-tenant memberships & roles, an in-app tenant switcher, and self-service workspace settings (name, slug). New organisations get their own isolated workspace on sign-up.
- **Configurable approvals & automations** — admins design **multi-step approval workflows** (ANY/ALL steps; role-, user- or PM-based approvers; conditions) that gate change requests and project closure, with an approvals inbox, SLA/escalation sweeps and approver delegation. A **no-code automation** builder turns events into notifications.
- **Public REST API & integrations** — scoped, bearer-authenticated **API keys** (per-key rate-limits + audit), **outbound webhooks** (HMAC-signed, retried), an **OpenAPI 3.1** spec with browsable docs, an **iCal** feed, **Slack/Teams** notification formats, and **Excel/CSV import** of tasks.
- **AI-assisted status narrative** — an optional, opt-in helper drafts project status commentary from live report data for the PM to review and edit (human-in-the-loop; off by default).
- **Direct messaging** — lightweight 1-to-1 chat between corporate users.
...
**Closure readiness gate** — closing a project runs a methodology-aware checklist. For **Agile/Hybrid** the hard blocks are *scope complete* (every backlog item **Done or Deferred** — judged per item, **not** by story points) and a *formal acceptance sign-off*; predictive projects gate on the WBS schedule being 100%. Everything else (change requests, risks, issues, cost, lessons) is an advisory warning. ADMIN/PMO can **force-close** with a mandatory, audited reason.
...
- **Modular API** under `/api/v1`: `auth`, `users`, `members`, `workspace`, `projects`, `charter`, `cost`, `ratecard`, `risk`, `issue`, `schedule`, `agile`, `timesheet`, `forecast`, `portfolio`, `evm`, `report`, `closeout`, `resource`, `attachment`, `audit`, `notification`, `messages`, `approvals`, `approval-workflows`, `automations`, `api-keys`, `webhooks`, `calendar`, `billing`, `export`, plus a public, key-authed API documented via **OpenAPI**.
